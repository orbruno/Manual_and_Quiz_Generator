...
from app.utils import call_llm, crawl_local_files
import tempfile
import tiktoken

MAX_MODEL_TOKENS   = 8192          # gpt-4 context window
ANSWER_RESERVE     = 1500          # leave room for model reply
...
class IdentifyAbstractions(Node):
    ...
    def prep(self, shared):
        files_data          = shared["files_content"]
        use_cache           = shared.get("use_cache", True)
        max_abstraction_num = shared.get("max_abstraction_num", 10)
        manual_focus        = shared.get("prompt",
                             "Make a general summary of the provided document")

        # ----------  1 ▸ work out static-tokens ------------------------
        prompt_header = (
            "The following document has the purpose: "
            f"`{manual_focus}`.\n\n"
            "Files Context:\n"
        )
        static_tokens = len(ENC.encode(prompt_header))
        # we will add the file-listing later – estimate 200 tokens
        static_tokens += 200

        # ----------  2 ▸ compute content-budget ------------------------
        content_budget = MAX_MODEL_TOKENS - ANSWER_RESERVE - static_tokens
        if content_budget <= 0:
            raise ValueError("Not enough room for any content with current settings")

        # ----------  3 ▸ build context within budget ------------------
        context, file_info = self._collect_chunks(
            files_data,
    ...
    # ----------  helper: collect chunks --------------------------------
    def _collect_chunks(self, files_data, budget_tokens, max_chunk_tokens):
        context      = []
        file_info    = []
        total_tokens = 0
        index_ctr    = 0
        ...
        for path, content in files_data:
            ...
            for start in range(0, len(tokens), max_chunk_tokens):
                ...
                if total_tokens + len(chunk) > budget_tokens:
                    # budget exhausted – stop collecting
                    print(f"Token budget reached at {total_tokens}/{budget_tokens}")
                    return "".join(context), file_info

                chunk_text = ENC.decode(chunk)
                context.append(
                    f"--- File Index {index_ctr}: {path} ---\n{chunk_text}\n\n"
                )
                file_info.append((index_ctr, path))
        ...
        #     # Ensure the entry has exactly two elements (path, content)
        #     if not isinstance(file_entry, tuple) or len(file_entry) != 2:
        #         print(f"Skipping invalid entry at index {i}: {file_entry}")
        #         continue

        #     path, content = file_entry

        #     # Debug: Check content length
        #     print(f"Processing file {path}, Content length: {len(content)}")

        #     entry = f"--- File Index {i}: {path} ---\n{content}\n\n"
        #     entry_tokens = len(encoder.encode(entry))

        #     # Debug: Check token count
        #     print(f"Entry tokens: {entry_tokens}, Remaining tokens: {max_tokens - total_tokens}")

        #     # If adding this entry exceeds the max tokens, stop
        #     if total_tokens + entry_tokens > max_tokens:
        #         print(f"Token limit reached at index {i}. Remaining tokens: {max_tokens - total_tokens}")
        #         break

        #     # Add to context and file info
        #     context += entry
    ...
    def exec(self, prep_res):
        ...
        # ✅ MODIFIED: Context now respects token limit
        prompt = f"""
The following document has the purpose: `{manual_focus}`.

Files Context:
{context}

Analyze the document context.
Identify the top 5-{max_abstraction_num} core abstractions to help those new to the topic.
...
File indices and paths present in the context:
{file_listing_for_prompt}
...
```"""
        ...
        print(f"[DEBUG] Prompt tokens: {used} / 8192")
        # ----------------------------

        # ✅ ADDED: Pass token management to LLM call
        ...
        for item in abstractions:
            ...
            # ✅ MODIFIED: Include validated indices only
            item["files"] = sorted(list(set(validated_indices)))

            # ✅ MODIFIED: Store only the required fields
    ...
    def post(self, shared, prep_res, exec_res):
        # ✅ MODIFIED: Store the final validated abstractions in shared store
